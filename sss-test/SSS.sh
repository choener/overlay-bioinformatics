...
if [ "$FORMAT" == "fasta" ]; then 
	...
	perl $script_dir/alignment_changes-nogap2.pl $familyID.alg-twoline 0.6 > $familyID.changes; 
...
if [ $FORMAT == "aligned" ]; then
	...
	echo 'Calculating alignment changes'
	perl $script_dir/alignment_changes-nogap2.pl $familyID-twoline 0.6 > $familyID.changes; 
...
while read species
  do
	...
	#This block applies the Benjamini-Hochberg method for multiple observation correction
